...
async function run() {
  ...
  console.log('[seed-user] Conectado ao banco');

  const email = 'empresa@busconnect.com';
  const senha = 'senha123';
  const hash = await bcrypt.hash(senha, 10);

  // Remove se ja existe (para poder reexecutar)
  ...
  const { rows } = await client.query(
    ...
    ['Empresa Teste', email, hash]
  );

  console.log('');
  console.log('===========================================');
  console.log('  USUARIO CRIADO PARA TESTE');
  console.log('===========================================');
